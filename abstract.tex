...
\sloppy
To address these problems, we develop \cheetah{}, a profiler that detects false sharing both {\bf efficiently} and {\bf effectively}. \cheetah{} leverages the lightweight hardware performance monitoring units (PMUs) that are available on most modern CPU architectures to sample memory accesses. \cheetah{} designs the first systematic approach that can quantify the optimization potential of  false sharing instances without actual fixes, based on the latency information of every memory access. \cheetah{} precisely reports false sharing and provides insightful optimization guidance for programmers, while adding less than $7\%$ runtime overhead on average. \Cheetah{} is ready for real deployment. 
