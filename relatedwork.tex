...
\cheetah{} proposes to predict the potential performance improvement after fixing an existing false sharing problem, by utilizing the access cycles provided by the PMU hardware. Prior work points out variables and instructions suffering with high access latency by utilizing the latency of every memory access~\cite{ibs-sc, ibs-pact}. But we did not find any work that is close to our target: using the latency information to predict the performance impact of fixing an existing false sharing instance, without actual fixes.
