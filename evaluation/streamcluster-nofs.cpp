...
#define CACHE_LINE 128 // cache line in byte
...
static long *switch_membership; //whether to switch membership in pgain
...
double pgain(long x, Points *points, double z, long int *numcenters)
{
  ...
  */
  ...
  memset(switch_membership, 0, points->num*sizeof(long));
...
double pgain(long x, Points *points, double z, long int *numcenters, int pid, pthread_barrier_t* barrier)
{
  ...
  */
  ...
  //now we finish building the table. clear the working memory.
  memset(switch_membership + k1, 0, (k2-k1)*sizeof(long));
...
#else
  	fprintf(stderr, "points.num %ld\n", points.num);
	  switch_membership = (long*)malloc(points.num*sizeof(long));
...
#else
	fprintf(stderr, "centers.num is %ld sizeoflonglong %d\n", centers.num, sizeof(long long));
  switch_membership = (long*)malloc(centers.num*sizeof(long));
