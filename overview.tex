...
\todo{Shadow Memory mechanism}

Why we should use the shadow memory? In order to use that, we have to intercept the memory allocations and deallocations. Intercepting the memory allocation and deallocation is also very important to precisely identify where the problem is from. For example, for heap objects, it is very helpful if a detect tool can point out where a heap object is coming from. 
